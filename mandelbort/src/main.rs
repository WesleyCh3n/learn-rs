...
fn main() {
    ...
    if args.len() != 5 {
        ...
        eprintln!(
            "Example: {} mandal.png 1000x750 -1.20,0.35 -1.0,0.20",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    let upper_left =
        parse_complex(&args[3]).expect("error parsing upper left conner point");
    let lower_right =
        parse_complex(&args[4]).expect("error parsing lower left conner point");
